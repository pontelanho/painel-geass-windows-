...
def display_placa_intel(j):
    ...
    print(f"\n{Colors.PURPLE}=== [ GEASS PLATE - VEHICLE TRACKER ] ==={Colors.RESET}")
    
    if not resultado:
        print(f"{Colors.RED}[!] Geass hasn't found data for this plate.{Colors.RESET}")
        return
    ...
    print(f"\n{Colors.PURPLE}--- [ GEASS PLATE - FUEL INFO ] ---")
...
def placa_lookup(placa):
    url = "https://xbuscas.net/api/marketplace/v1/products/placa/consult"
    ...
    print(f"\n{Colors.PURPLE}[!] TRACKING VEHICLE'S PLATE: {target}...{Colors.RESET}")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"{Colors.RED}[!] Erro {response.status_code}: Falha na consulta de veículos.{Colors.RESET}")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        ...
        print(f"{Colors.RED}[!] Erro: Informe a placa (Nacional ou Mercosul).{Colors.RESET}")
